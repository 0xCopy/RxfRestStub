 ...
 */
class Shard {
  private final AsyncSingletonServer asyncSingletonServer = new AsyncSingletonServer() {
  };
  ...
  public void enqueue(SelectableChannel channel, int op, Object... s) {
    assert channel != null && !AsyncSingletonServer.killswitch.get() : "Server appears to have shut down, cannot enqueue";
    /*
     * if (Thread.currentThread() == selectorThread) try { channel.register(AsioVisitor.Helper.getSelector(), op, s); }
     * catch (ClosedChannelException e) { e.printStackTrace(); } else
     */
    {
            ...
            try {
              ...
              if (key.isValid() && key.isWritable()) {
                if (((SocketChannel) channel).socket().isOutputShutdown()) {
                  key.cancel();// nio boogs otherwise
                } else {
                  m.onWrite(key);
                }
              }
  ...
  public AsyncSingletonServer getAsyncSingletonServer() {
    return asyncSingletonServer;
  }
